...
const NodeJsServiceOptions = require('@gobytecoin/dp-services-ctl/lib/services/node/NodeJsServiceOptions');
...
class InsightUIOptions extends NodeJsServiceOptions {
  ...
  mergeWithDefaultOptions(...customOptions) {
    ...
    const defaultServiceOptions = {
      config: {
        ...
        services: [
          'gobyted',
          '@gobytecoin/insight-api',
          '@gobytecoin/insight-ui',
          'web',
        ],
      },
    ...
    const defaultContainerOptions = {
      image: 'gobytecoin/insight',
      network: {
        name: 'gobyte_test_network',
        driver: 'bridge',
      },
      ports: [
        `${port}:${port}`,
      ...
      cmd: [
        '/insight/bin/gobytecore-node',
        'start',
      ],
      volumes: [...InsightUIOptions.defaultCustomOptions.container.volumes,
        `${configFile}:/insight/gobytecore-node.json`,
      ],
    };
  ...
  getConfigPath() {
    ...
    return path.join(tmpPath, 'bitcore-node-goobyte.json');
  }
